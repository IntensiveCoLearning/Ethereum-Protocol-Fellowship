...
Bob 確保 Alice 的公鑰是正確的，這樣才能驗證簽章的真實性。  
...
- 存款流程**不變**，工具如staking-deposit-cli可繼續使用。
- **EIP-6110**實施後，存款資料可**即時上鏈**，大幅縮短確認時間。
- 預期**20分鐘內**，存款資料就會出現在鏈上。

#### 2. Validator啟動等待時間

- **存款確認加速** ≠ 立即啟動。
- **啟動排隊（Deposit Queue）**機制仍然存在。
- EIP-6110只是**加快存款被鏈看見的速度**，不改變啟動排隊邏輯。

#### 3. Withdrawal Credentials類型變化

| 類型 | 0x00 | 0x01 | 0x02 |
|---|---|---|---|
| 存款上限 | 32 ETH | 32 ETH | 2048 ETH |
| 退出控制 | BLS簽名 | 執行層地址控制 | 執行層地址控制 |
| 合併功能 | ❌ | ❌ | ✅ |
| 直接存款支援 | ❌ | ✅ | ✅ |

- Pectra前：
    - 0x00：BLS簽名控制
    - 0x01：執行層（EL）地址控制，32 ETH上限
- Pectra後：
    - 新增0x02：EL地址控制，**2048 ETH上限**，支援**合併多個Validator**。

#### 4. 如何升級Withdrawal Credentials

- **0x00 → 0x02**
    1. 先BLS變更，轉為0x01。
    2. 再執行合併（Consolidation），升級到0x02。
    3. 或者直接退出，再用0x02重新存款。

- **0x01 → 0x02**
    - 直接執行**自我合併**（Self-Consolidation），升級為0x02。

#### 5. 使用0x02存款

- staking-cli已支援：
    - `--compounding`：使用0x02
    - `--amount`：自由設定存款金額（高於32 ETH也可）
- 官方staking-deposit-cli即將支援，預計在Pectra前完成。
- 存款流程：
    1. 生成deposit資料檔
    2. 提交到**Dora Submit Deposits**或**官方Launchpad**（近期支援）

#### 6. 0x02帶來的優勢

- 單一Validator可**質押高達2048 ETH**。
- 支援**合併**多個Validator，方便管理。
- 質押與退出**完全由執行層控制**，不再受限於共識層流程。

Pectra升級是以太坊對質押機制的一次重大優化，帶來多項針對Stakers（質押者）的重要改變。驗證人啟用流程將變得更快、更具彈性。過去質押32枚ETH啟用一個新驗證人時，存款處理可能需要數小時，加上排隊等待，整體啟用過程相當冗長。Pectra中的EIP-6110縮短這個流程，讓存款幾乎可以立即被鏈上處理，讓驗證人更快上線。而EIP-7251則打破過去單一驗證人固定32ETH的上限，允許每個驗證人最多持有2048ETH，這代表大戶可以將多筆資金集中於同一個驗證人，省去過去拆分多個節點的麻煩，降低運營與管理成本，對大型Staker非常有利。

第二個改變是自動複利功能的加入。Pectra引入0x02類型的提領憑證，讓質押獎勵可以自動滾入本金，形成複利效果，讓資金增長更有效率。儘管這些累積的複利獎勵不會自動提領，若需要領回仍需支付Gas費，但這對於長期質押者來說，無疑是省事又有利的設計。過去驗證人若違規遭Slashing，初犯會直接被罰1/32的餘額，相當嚴重。Pectra將這項懲罰大幅降低至1/4096，雖然違規仍需負責，但減少了因意外或技術疏失造成的重大經濟損失。

Pectra也改變了質押者的退出權限，帶來前所未有的資產自主權。過去使用質押服務（如交易所或第三方平台），若想退出，必須由服務商事先簽署退出資訊，這等於質押者的資產掌控權受制於他人。而Pectra引入EIP-7002，讓驗證人可以直接透過執行層（Execution Layer）發動自主退出，不再依賴服務商，真正實現質押者對自己資產的完全掌控。這不僅提升了資安層面的保障，也減少營運商倒閉或惡意阻撓時，資產無法領回的風險。

Pectra讓質押更快速、更靈活、更自主，無論是大額質押者、小額參與者，甚至依賴第三方服務的用戶，都能感受到更友善的使用體驗，讓以太坊的質押生態更加成熟與普及。

**近況**：Pectra分兩個階段實施，第一階段在2025年3月重點推動帳戶抽象與質押上限提升等功能，第二階段則預計在2025年底或2026年初上線，進一步引入Verkle Tree與PeerDAS，完整強化資料儲存與擴展能力。

不過Pectra也面臨一些挑戰，包括節點軟體的多樣性不足，導致主流客戶端若出現Bug，可能危及整體網路穩定。另外，質押上限提升也可能讓少數大型服務商集中過多權力，增加中心化風險。2025年2月，Pectra在Holesky測試網上遇到無法達成最終確認（Finality）的技術問題，雖然這是測試階段的正常現象，但仍顯示Pectra上線前需要進一步優化。
